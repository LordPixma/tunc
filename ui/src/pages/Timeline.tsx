...
export function Timeline() {
  ...
  async function handleAddContent() {
    try {
      const res = await fetch(`/capsule/${id}/item`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ message: 'New content' }),
      });
      if (!res.ok) {
  ...
  return <div className="pb-6">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={handleAddContent} className="flex items-center bg-gradient-to-r from-orange-500 to-pink-500 text-white px-4 py-2 rounded-full text-sm font-medium">
